...
Rugalarizacion_Lasso <- function(modelo){
...
graficacion_lasso <- function(model_lasso){
...
```{r}
simple <- Rugalarizacion_Lasso(" . ")
graficacion_lasso(simple$modelo_lasso )
...
```{r}
 ...
 df_resultados <- rbind(df_resultados, nueva_fila_resultados( "Lasso", AIC_lasso, 7))
...
```{r}
modelo_conpleto_step <- step(
  object = modeloInteracciones,
  direction = "backward",
  scope = list(upper = modeloInteracciones, lower = ~1),
  trace = FALSE
)

df_resultados <- rbind(df_resultados, nueva_fila_resultados( "Stepwise_Backward Interacciones", AIC(modelo_conpleto_step), length(modelo_conpleto_step$coefficients)-1))
```
